...
if( ! class_exists( 'war_theme ' ) ):
    ...
    class war_theme {
        ...
        private function war_default_secondary_scripts(){
            return array(
                // 'war_post_child_js' => [
        ...
        private function war_filter_secondary_scripts(){
            $this->war_secondary_scripts = apply_filters( 'war_secondary_scripts', $this->war_default_secondary_scripts() );
        }
        ...
        private function war_register_scripts(){
            array_walk( $this->war_priority_scripts, function( $v, $k ){
                if( ! $v || empty( $v ) ) return;
                $this->war_registrar( $k, $v );
            });
            array_walk( $this->war_child_scripts, function( $v, $k ){
                if( ! $v || empty( $v ) ) return;
                $this->war_registrar( $k, $v );
            });
            array_walk( $this->war_secondary_scripts, function( $v, $k ){
                if( ! $v || empty( $v ) ) return;
                $this->war_registrar( $k, $v );
            });
        }
        ...
        private function war_registrar( $name = false, $item = [], $is_style = false ){
            ...
            if( $is_style ){
                ...
                if( ! $depends ) $depends = [ 'war_extend_init' ];
